...
Dart/Flutter client: [`configwire/dart`](https://github.com/configwire/dart)
([pub.dev/packages/configwire](https://pub.dev/packages/configwire)) —
pure-Dart, persistence is bring-your-own CacheStore.

Wire details live in `docs/CONTRACT.md`. Operator security notes live in
`docs/SECURITY.md`. Key rotation live in `docs/ROTATION.md`.

## Prereqs

- Go (version floor per `go.mod`)
- `make`, `bash`, `curl`, `python3`

## Serving (cwd rule)

> Serve MUST run from the repo root so `./pb_public` resolves. Started from
...
That is shorthand for `go run . serve` (default `127.0.0.1:8090`,
data in `./pb_data`). Flags pass through Go, not make:
...
Full client docs: https://github.com/configwire/dart.
...
| `make test` | Go build+vet+test (exit 0) |
| `make lint` | `gofmt` check + `go vet` |
| `make e2e` | `bash scripts/e2e.sh` |

CI (`.github/workflows/ci.yml`) runs Go build/vet/test plus `make lint`
and `make test` on push/PR.
First run needs network for `go mod download`; after priming,
`make test` needs nothing beyond the module cache.
